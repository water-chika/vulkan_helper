...
concept instance = requires(Instance instance) {
                     {
                       ...
                       } -> std::convertible_to<vk::Instance>;
                   };
namespace instance_helper {
template <class T>
concept get_extensions = requires(T t) { t.get_extensions(); };
}
template <class T>
concept physical_device = requires(T t) {
                            {
                              t.get_physical_device()
                              } -> std::convertible_to<vk::PhysicalDevice>;
                          };
template <class T>
...
public:
  ...
  ~add_instance() { m_instance.destroy(); }
  auto get_instance() { return m_instance; }

private:
  vk::Instance m_instance;
};
template <class T> class add_empty_extensions : public T {
public:
  auto get_extensions() { return std::vector<std::string>{}; }
};
template <class T> class add_surface_extension : public T {
public:
  auto get_extensions() {
...
template <concept_helper::device Device> class add_fence : public Device {
public:
  using parent = Device;
  add_fence() : Device{} {
...
template <class T> class add_recreate_surface_for_swapchain : public T {
public:
  using parent = T;
...
public:
  ...
  auto get_swapchain() { return m_swapchain; }
...
template <class T> class rename_images_views_to_depth_images_views : public T {
public:
  using parent = T;
  auto get_depth_images_views() { return parent::get_images_views(); }
};
...
public:
  ...
  auto get_swapchain_image_views() { return m_views; }

private:
  std::vector<vk::ImageView> m_views;
};
template <class T> class add_recreate_surface_for_swapchain_images : public T {
public:
  using parent = T;
  void recreate_surface() {
    parent::recreate_surface();
...
public:
  ...
  auto get_swapchain_image(uint32_t index) { return m_images[index]; }
  auto get_swapchain_images() { return m_images; }
...
public:
  using parent = T;
  auto get_depth_image_format() { return parent::get_image_format(); }
};
...
public:
  ...
  void create_images() {
    ...
    std::ranges::for_each(m_images, [device, extent, format, image_type,
                                     queue_family_index, image_usage, samples,
                                     tiling](vk::Image &image) {
      image =
          device.createImage(vk::ImageCreateInfo{}
                                 .setArrayLayers(1)
                                 .setExtent(extent)
                                 .setFormat(format)
  ...
  void destroy_images() {
    vk::Device device = parent::get_device();
    std::ranges::for_each(
        m_images, [device](vk::Image image) { device.destroyImage(image); });
  }
  auto get_images() { return m_images; }

private:
  std::vector<vk::Image> m_images;
};

template <class T> class add_image_memory : public T {
public:
  using parent = T;
  add_image_memory() {
    vk::Device device = parent::get_device();
    ...
    vk::MemoryPropertyFlags memory_properties =
        parent::get_image_memory_properties();

    auto memory_requirements = device.getImageMemoryRequirements(image);
    uint32_t memory_type_index = parent::find_properties(
...
public:
  ...
  auto create() {
    ...
    vk::MemoryPropertyFlags memory_properties =
        parent::get_buffer_memory_properties();
...
public:
  ...
  auto get_buffer_memory_ptr_vector() { return m_ptrs; }
...
public:
  ...
  add_buffer_memory_vector() {
    ...
    auto memory_requirements = device.getBufferMemoryRequirements(buffers[0]);
    uint32_t memory_type_index = parent::find_properties(
        memory_requirements.memoryTypeBits, memory_properties);
    m_memory.resize(buffers.size());
    std::ranges::transform(
        buffers, m_memory.begin(),
        [device, memory_type_index, memory_requirements,
         buffers](auto &buffer) {
  ...
  ~add_buffer_memory_vector() {
    vk::Device device = parent::get_device();
    std::ranges::for_each(
        m_memory, [device](auto &memory) { device.freeMemory(memory); });
  }
  auto get_buffer_memory_vector() { return m_memory; }
...
public:
  ...
  auto get_buffer_memory() { return m_memory; }

private:
  vk::DeviceMemory m_memory;
};
template <class T> class add_recreate_surface_for_images_memories : public T {
public:
  using parent = T;
  void recreate_surface() {
...
public:
  ...
  void destroy_images_memories() {
    vk::Device device = parent::get_device();
    std::ranges::for_each(m_memories, [device](vk::DeviceMemory memory) {
      device.freeMemory(memory);
    });
  }
  auto get_images_memories() { return m_memories; }
...
public:
  ...
  auto get_physical_device_memory_properties() { return m_properties; }
...
public:
  using parent = T;
  uint32_t find_properties(uint32_t memory_type_bits_requirements,
                           vk::MemoryPropertyFlags required_property) {
    vk::PhysicalDeviceMemoryProperties memory_properties =
        parent::get_physical_device_memory_properties();
    ...
    for (uint32_t memoryIndex = 0; memoryIndex < memory_count; memoryIndex++) {
      ...
      const bool is_required_memory_type =
          memory_type_bits_requirements & memoryTypeBits;
...
public:
  ...
  add_command_pool() {
    ...
    m_pool = device.createCommandPool(
        vk::CommandPoolCreateInfo{}.setQueueFamilyIndex(queue_family_index));
  }
  ~add_command_pool() {
    vk::Device device = parent::get_device();
    device.destroyCommandPool(m_pool);
  }
  auto get_command_pool() { return m_pool; }

private:
  vk::CommandPool m_pool;
};
template <class T> class add_command_buffer : public T {
public:
  using parent = T;
  add_command_buffer() {
    vk::Device device = parent::get_device();
    ...
    m_buffer = device.allocateCommandBuffers(vk::CommandBufferAllocateInfo{}
                                                 .setCommandPool(pool)
  ...
  auto get_command_buffer() { return m_buffer; }
...
public:
  ...
  auto get_swapchain_command_buffers() { return m_buffers; }
  auto get_swapchain_command_buffer(uint32_t i) { return m_buffers[i]; }
  ...
  clear_color_value_type get_format_clear_color_value_type(vk::Format f) {
    ...
    if (!types.contains(f)) {
      throw std::runtime_error{
          "this format does not support clear color value"};
    }
    return types[f];
  }
};
template <class T> class add_recreate_surface_for_framebuffers : public T {
public:
  using parent = T;
  void recreate_surface() {
...
template <class T> class add_framebuffers : public T {
public:
  using parent = T;
  ...
  void create_framebuffers() {
    ...
    std::ranges::transform(
        swapchain_image_views, m_framebuffers.begin(),
...
template <class T> class rename_vertex_buffer_to_buffer : public T {
public:
  using parent = T;
  auto get_buffer() { return parent::get_vertex_buffer(); }
};
template <vk::MemoryPropertyFlagBits Property, class T>
...
public:
  using parent = T;
  auto get_buffer_data() { return parent::get_vertex_buffer_data(); }
};
...
class add_buffer_memory_with_data_copy
    : public copy_buffer_data<add_buffer_memory<set_buffer_memory_properties<
          vk::MemoryPropertyFlagBits::eHostVisible, T>>> {};
...
public:
  using parent = T;
  auto get_vertex_buffer_vector() { return parent::get_buffer_vector(); }
};
template <class T>
class rename_buffer_vector_to_uniform_buffer_vector : public T {
public:
  using parent = T;
  auto get_uniform_buffer_vector() { return parent::get_buffer_vector(); }
};
template <class T>
...
template <class T>
class rename_buffer_memory_vector_to_uniform_upload_buffer_memory_vector
    : public T {
public:
  using parent = T;
...
template <class T>
class rename_buffer_memory_ptr_vector_to_vertex_buffer_memory_ptr_vector
    : public T {
public:
  using parent = T;
...
template <class T>
class rename_buffer_memory_ptr_vector_to_uniform_upload_buffer_memory_ptr_vector
    : public T {
public:
  using parent = T;
...
template <class T> class rename_buffer_to_index_buffer : public T {
public:
  using parent = T;
  auto get_index_buffer() { return parent::get_buffer(); }
};
template <class T> class rename_buffer_to_vertex_buffer : public T {
public:
  using parent = T;
  auto get_vertex_buffer() { return parent::get_buffer(); }
};
...
template <class T> class add_buffer_create : public T {
public:
  using parent = T;
  ...
  auto create() {
    ...
    return device.createBuffer(vk::BufferCreateInfo{}
                                   .setQueueFamilyIndices(queue_family_index)
                                   .setSize(size)
                                   .setUsage(usage));
  }
...
public:
  ...
  add_vector_for() {
    ...
    std::ranges::for_each(m_members,
                          [this](auto &member) { member = parent::create(); });
  }
  ~add_vector_for() {
    std::ranges::for_each(m_members,
                          [this](auto &member) { parent::destroy(member); });
  }
  auto get_vector() { return m_members; }
...
template <class T> class add_draw : public T {
public:
  using parent = T;
  void draw() {
    vk::Device device = parent::get_device();
...
template <class T> class add_acquire_next_image_fences : public T {
public:
  using parent = T;
  add_acquire_next_image_fences() {
    ...
    std::ranges::for_each(m_fences, [device](vk::Fence &fence) {
  ...
  ~add_acquire_next_image_fences() {
    vk::Device device = parent::get_device();
    std::ranges::for_each(
        m_fences, [device](vk::Fence fence) { device.destroyFence(fence); });
  }
  auto get_acquire_next_image_fences() { return m_fences; }
...
public:
  ...
  add_draw_semaphores() {
    ...
    std::ranges::for_each(m_semaphores, [device](vk::Semaphore &semaphore) {
  ...
  auto get_draw_image_semaphore(uint32_t i) { return m_semaphores[i]; }
...
template <class T> class add_recreate_surface_for_pipeline : public T {
public:
  using parent = T;
  void recreate_surface() {
...
public:
  ...
  void create_pipeline() {
    ...
    vk::PipelineColorBlendStateCreateInfo color_blend_state =
        parent::get_pipeline_color_blend_state_create_info();
    vk::PipelineDepthStencilStateCreateInfo depth_stencil_state =
        parent::get_pipeline_depth_stencil_state_create_info();
    vk::PipelineDynamicStateCreateInfo dynamic_state =
        parent::get_pipeline_dynamic_state_create_info();
    vk::PipelineInputAssemblyStateCreateInfo input_assembly_state =
        parent::get_pipeline_input_assembly_state_create_info();
  ...
  auto get_pipeline() { return m_pipeline; }

private:
  vk::Pipeline m_pipeline;
};
template <uint32_t Subpass, class T> class set_subpass : public T {
public:
  auto get_subpass() { return Subpass; }
};
template <class T> class add_empty_attachments : public T {
public:
  auto get_attachments() { return std::vector<vk::AttachmentDescription>{}; }
};
template <class T> class add_depth_attachment : public T {
public:
  using parent = T;
...
template <class T> class add_attachment : public T {
public:
  using parent = T;
  auto get_attachments() {
    auto attachments = parent::get_attachments();
...
template <class T> class add_subpass_dependency : public T {
public:
  using parent = T;
...
template <class T> class add_empty_subpass_dependencies : public T {
public:
  auto get_subpass_dependencies() {
...
template <class T> class add_scissor_equal_swapchain_extent : public T {
public:
  using parent = T;
  auto get_scissors() {
    ...
    scissors.emplace_back(vk::Rect2D{}.setOffset({}).setExtent(extent));
    return scissors;
  }
};
template <class T> class add_scissor_equal_surface_rect : public T {
public:
  using parent = T;
  auto get_scissors() {
    ...
    scissors.emplace_back(
        vk::Rect2D{}.setOffset({}).setExtent(surface_cap.currentExtent));
    return scissors;
  }
};
template <class T>
...
public:
  ...
  auto get_surface_capabilities() { return m_capabilities; }
...
public:
  ...
  void destroy() {}
  auto get_physical_device_support_surface() { return m_support; }

private:
  bool m_support;
};
template <class T> class add_empty_scissors : public T {
public:
  auto get_scissors() { return std::vector<vk::Rect2D>{}; }
};
template <class T> class add_viewport : public T {
public:
  using parent = T;
  auto get_viewports() {
...
template <class T> class add_viewport_equal_swapchain_image_rect : public T {
public:
  using parent = T;
  auto get_viewports() {
    auto viewports = parent::get_viewports();
...
template <float X, class T> class set_viewport_x : public T {
public:
  auto get_viewport_x() { return X; }
};
template <float Y, class T> class set_viewport_y : public T {
public:
  auto get_viewport_y() { return Y; }
};
template <float Width, class T> class set_viewport_width : public T {
public:
  auto get_viewport_width() { return Width; }
};
template <float Height, class T> class set_viewport_height : public T {
public:
  auto get_viewport_height() { return Height; }
};
template <float Min_depth, class T> class set_viewport_min_depth : public T {
public:
  auto get_viewport_min_depth() { return Min_depth; }
};
template <float Max_depth, class T> class set_viewport_max_depth : public T {
public:
  auto get_viewport_max_depth() { return Max_depth; }
};
template <class T> class add_empty_viewports : public T {
public:
  auto get_viewports() { return std::vector<vk::Viewport>{}; }
};
...
public:
  auto get_vertex_input_attribute_format() { return Format; }
};
template <class T> class add_vertex_attribute_description : public T {
public:
  using parent = T;
  auto get_vertex_attribute_descriptions() {
    ...
    descs.emplace_back(vk::VertexInputAttributeDescription{}
                           .setLocation(0)
                           .setBinding(0)
                           .setOffset(0)
                           .setFormat(format));
...
template <class T> class add_empty_vertex_attribute_descriptions : public T {
public:
  auto get_vertex_attribute_descriptions() {
...
public:
  auto get_input_rate() { return InputRate; }
};
template <uint32_t Binding, class T> class set_binding : public T {
public:
  auto get_binding() { return Binding; }
};
template <uint32_t Stride, class T> class set_stride : public T {
public:
  auto get_stride() { return Stride; }
};
template <class T> class add_vertex_binding_description : public T {
public:
  using parent = T;
  auto get_vertex_binding_descriptions() {
    ...
    descs.emplace_back(vk::VertexInputBindingDescription{}
                           .setBinding(binding)
...
template <class T> class add_empty_binding_descriptions : public T {
public:
...
public:
  auto get_pipeline_tessellation_state_create_info() {
    return vk::PipelineTessellationStateCreateInfo{}.setPatchControlPoints(
        Count);
  }
};
template <class T> class add_pipeline_stage_to_stages : public T {
public:
  using parent = T;
...
public:
  auto get_shader_stage() { return Shader_stage; }
};
template <class T> class set_shader_entry_name_with_main : public T {
public:
  auto get_shader_entry_name() { return std::string{"main"}; }
};
...
public:
  ...
  add_shader_module() {
    ...
    m_module =
        device.createShaderModule(vk::ShaderModuleCreateInfo{}.setCode(code));
  }
  ~add_shader_module() {
    vk::Device device = parent::get_device();
    device.destroyShaderModule(m_module);
  }
  auto get_shader_module() { return m_module; }

private:
  vk::ShaderModule m_module;
};
template <class T> class add_spirv_code : public T {
public:
  using parent = T;
  auto get_spirv_code() {
...
template <class T> class adapte_map_file_to_spirv_code : public T {
public:
  using parent = T;
  auto get_code_pointer() { return parent::get_mapped_pointer(); }
  auto get_size_in_bytes() { return parent::get_file_size(); }
};
#if WIN32
...
#endif
template <class T> class add_vertex_shader_path : public T {
public:
  auto get_file_path() { return std::filesystem::path{"vert.spv"}; }
};
template <class T> class add_fragment_shader_path : public T {
public:
  auto get_file_path() { return std::filesystem::path{"frag.spv"}; }
};
template <class T> class add_empty_pipeline_stages : public T {
public:
  auto get_pipeline_stages() {
...
template <class T> class disable_pipeline_multisample : public T {
public:
  auto get_pipeline_multisample_state_create_info() {
    return vk::PipelineMultisampleStateCreateInfo{}.setRasterizationSamples(
...
public:
  auto get_pipeline_input_assembly_state_create_info() {
    return vk::PipelineInputAssemblyStateCreateInfo{}.setTopology(Topology);
  }
};
template <class T> class disable_pipeline_dynamic : public T {
public:
  auto get_pipeline_dynamic_state_create_info() {
    return vk::PipelineDynamicStateCreateInfo{};
  }
};
template <class T> class enable_pipeline_depth_test : public T {
public:
...
template <class T> class disable_pipeline_depth_stencil : public T {
public:
  auto get_pipeline_depth_stencil_state_create_info() {
    return vk::PipelineDepthStencilStateCreateInfo{}
...
public:
  ...
  auto get_pipeline_color_blend_attachment_states() {
    ...
    attachments[AttachmentIndex].setBlendEnable(false);
    return attachments;
  }
};
...
public:
  auto get_pipeline_color_blend_attachment_states() {
    auto states =
        std::vector<vk::PipelineColorBlendAttachmentState>(AttachmentCount);
...
template <class T> class add_single_descriptor_set_layout : public T {
public:
  using parent = T;
...
public:
  ...
  auto get_descriptor_set_layout() { return m_layout; }

private:
  vk::DescriptorSetLayout m_layout;
};
template <class T> class add_descriptor_set_layout_binding : public T {
public:
  using parent = T;
  ...
  auto get_descriptor_set_layout_binding() { return m_binding; }
...
public:
  auto get_descriptor_count() { return Descriptor_count; }
};
template <vk::DescriptorType Descriptor_type, class T>
class set_descriptor_type : public T {
public:
  auto get_descriptor_type() { return Descriptor_type; }
};
template <class T> class add_empty_immutable_samplers : public T {
public:
  auto get_immutable_samplers() { return std::vector<vk::Sampler>{}; }
};
template <class T> class add_recreate_surface : public T {
public:
  using parent = T;
  void recreate_surface() {
...
template <class Instance>
concept instance =
    requires(Instance instance) { instance.get_vulkan_instance(); };
template <class Instance>
concept instance_help_functions =
    requires(Instance instance) { instance.get_first_physical_device(); };
namespace instance_helper {
template <class T>
concept get_extensions = requires(T t) { t.get_extensions(); };
}
...
public:
  VkPhysicalDevice get_first_physical_device() {
    ...
    auto res = vkEnumeratePhysicalDevices(instance::get_vulkan_instance(),
                                          &count, &physical_device);
    return physical_device;
  }
  template <int NUM>
  uint32_t enumerate_limit_physical_devices(
      std::array<VkPhysicalDevice, NUM> &physical_devices) {
    uint32_t count = physical_devices.size();
    auto res = vkEnumeratePhysicalDevices(instance::get_vulkan_instance(),
                                          &count, physical_devices.data());
...
public:
  ...
  ~instance() { vkDestroyInstance(m_instance, NULL); }

  auto get_vulkan_instance() { return m_instance; }
...
template <class T> class info_chain {
public:
private:
  T info;
...
public:
  uint32_t
  find_queue_family_if(std::predicate<VkQueueFamilyProperties> auto &&fun) {
    constexpr uint32_t COUNT = 8;
    std::array<VkQueueFamilyProperties, COUNT> properties{};
    uint32_t count = COUNT;
    vkGetPhysicalDeviceQueueFamilyProperties(
        physical_device::get_vulkan_physical_device(), &count,
        properties.data());
    assert(count > 0);
  ...
  auto get_physical_device_memory_properties() {
    ...
    vkGetPhysicalDeviceMemoryProperties(
        physical_device::get_vulkan_physical_device(), &properties);
    return properties;
  }
  ...
  VkDevice create_device(const device_create_info &info) {
    ...
    VkDevice device;
    auto res = vkCreateDevice(physical_device::get_vulkan_physical_device(),
                              &create_info, NULL, &device);
    if (res != VK_SUCCESS) {
...
uint32_t findProperties(VkPhysicalDeviceMemoryProperties memory_properties,
  ...
  for (uint32_t memoryIndex = 0; memoryIndex < memoryCount; memoryIndex++) {
    ...
    const bool isRequiredMemoryType =
        memoryTypeBitsRequirements & memoryTypeBits;
...
public:
  VkQueue get_device_queue(uint32_t queue_family_index, uint32_t queue_index) {
    VkQueue queue;
    vkGetDeviceQueue(device::get_vulkan_device(), queue_family_index,
                     queue_index, &queue);
    return queue;
  }
  VkFence create_fence() {
    ...
    VkFence fence;
    {
      ...
      auto res = vkCreateFence(device::get_vulkan_device(), &fence_create_info,
                               NULL, &fence);
      if (res != VK_SUCCESS) {
  ...
  VkShaderModule create_shader_module(const spirv_file &file) {
    ...
    auto res = vkCreateShaderModule(device::get_vulkan_device(), &create_info,
                                    NULL, &shader_module);
    if (res != VK_SUCCESS) {
  ...
  auto
  create_descriptor_set_layout(VkDescriptorSetLayoutCreateInfo *create_info) {

    VkDescriptorSetLayout descriptor_set_layout;
    auto res = vkCreateDescriptorSetLayout(
        device::get_vulkan_device(), create_info, NULL, &descriptor_set_layout);
    if (res != VK_SUCCESS) {
  ...
  auto create_pipeline_layout(VkDescriptorSetLayout descriptor_set_layout) {
    ...
    auto res = vkCreatePipelineLayout(device::get_vulkan_device(), &create_info,
  ...
  auto create_pipeline(VkShaderModule shader_module,
                       VkPipelineLayout pipeline_layout) {
    ...
    VkPipeline pipeline;
    auto res =
        vkCreateComputePipelines(device::get_vulkan_device(), VK_NULL_HANDLE, 1,
  ...
  auto create_command_pool(uint32_t queue_family_index) {
    ...
    auto res = vkCreateCommandPool(device::get_vulkan_device(), &create_info,
                                   NULL, &command_pool);
    if (res != VK_SUCCESS) {
  ...
  auto allocate_command_buffer(VkCommandPool command_pool) {
    ...
    auto ret = vkAllocateCommandBuffers(device::get_vulkan_device(), &info,
                                        &command_buffer);
    if (ret != VK_SUCCESS) {
  ...
  VkBuffer create_buffer(uint32_t queue_family_index, VkDeviceSize size,
    ...
    VkBuffer buffer;
    auto res = vkCreateBuffer(device::get_vulkan_device(), &create_info, NULL,
                              &buffer);
    if (res != VK_SUCCESS) {
  ...
  VkDeviceMemory
  alloc_device_memory(VkPhysicalDeviceMemoryProperties memory_properties,
                      VkBuffer buffer, VkMemoryPropertyFlags property) {
    VkMemoryRequirements requirements;
    vkGetBufferMemoryRequirements(device::get_vulkan_device(), buffer,
                                  &requirements);
    uint32_t memoryType = findProperties(memory_properties,
                                         requirements.memoryTypeBits, property);
    ...
    VkDeviceMemory device_memory{};
    {
      ...
      auto res = vkAllocateMemory(device::get_vulkan_device(), &info, NULL,
                                  &device_memory);
      if (res != VK_SUCCESS) {
  ...
  VkDeviceMemory
  alloc_device_memory(VkPhysicalDeviceMemoryProperties memory_properties,
                      VkImage image, VkMemoryPropertyFlags property) {
    VkMemoryRequirements requirements;
    vkGetImageMemoryRequirements(device::get_vulkan_device(), image,
                                 &requirements);
    uint32_t memoryType = findProperties(memory_properties,
                                         requirements.memoryTypeBits, property);
  ...
  void *map_device_memory(VkDeviceMemory device_memory, VkDeviceSize offset,
                          VkDeviceSize size) {
    void *ptr{};
    auto res = vkMapMemory(device::get_vulkan_device(), device_memory, offset,
                           size, 0, &ptr);
    if (res != VK_SUCCESS) {
  ...
  auto create_descriptor_pool(VkDescriptorPoolCreateInfo *create_info) {
    ...
    vkCreateDescriptorPool(device::get_vulkan_device(), create_info, NULL,
                           &descriptor_pool);
  ...
  auto allocate_descriptor_set(VkDescriptorPool descriptor_pool,
                               VkDescriptorSetLayout descriptor_set_layout) {
    ...
    auto res = vkAllocateDescriptorSets(device::get_vulkan_device(), &info,
                                        &descriptor_set);
    if (res != VK_SUCCESS) {
  ...
  void wait_for_fence(VkFence fence) {
    auto res = vkWaitForFences(device::get_vulkan_device(), 1, &fence, VK_TRUE,
                               UINT64_MAX);
    if (res != VK_SUCCESS) {
  ...
  void invalidate_mapped_memory_ranges(VkDeviceMemory memory,
    ...
    auto res = vkInvalidateMappedMemoryRanges(device::get_vulkan_device(), 1,
                                              &memory_range);
    if (res != VK_SUCCESS) {
  ...
  VkImage create_image(VkImageCreateInfo *create_info) {
    VkImage image{};
    VkResult res = vkCreateImage(device::get_vulkan_device(), create_info,
                                 nullptr, &image);
    if (res != VK_SUCCESS) {
  ...
  VkImageView create_image_view(VkImageViewCreateInfo *create_info) {
    VkImageView view{};
    VkResult res = vkCreateImageView(device::get_vulkan_device(), create_info,
                                     nullptr, &view);
    if (res != VK_SUCCESS) {
...
public:
  ...
  ~device() noexcept { vkDestroyDevice(m_device, nullptr); }
  device &operator=(const device &device) = delete;
  device &operator=(device &&device) = delete;

  VkDevice get_vulkan_device() { return m_device; }

private:
  VkDevice m_device;
};

template <class D> class command_pool : public D {
public:
  command_pool(std::invocable<D &> auto &&gen_command_pool)
      : m_command_pool{gen_command_pool(*this)} {}
  command_pool() = delete;
  command_pool(const command_pool &) = delete;
  command_pool(command_pool &&) = delete;
  ~command_pool() { D::destroy_command_pool(m_command_pool); }
...
template <class D> class fence : public D {
public:
  fence() : m_fence{D::create_fence()} {}
  ~fence() { D::destroy_fence(m_fence); }
  VkFence get_fence() { return m_fence; }

  void reset() { D::reset_fence(m_fence); }
  void wait_for() { D::wait_for_fence(m_fence); }

private:
  VkFence m_fence;
};

template <class D> class descriptor_set : public D {
public:
public:
  descriptor_set()
      : m_descriptor_set{
            D::allocate_descriptor_set(D::get_descriptor_set_layout())} {}

  auto get_descriptor_set() { return m_descriptor_set; }

private:
  VkDescriptorSet m_descriptor_set;
};

template <class D> class pipeline_layout : public D {
public:
  pipeline_layout()
      : m_pipeline_layout{
            D::create_pipeline_layout(D::get_descriptor_set_layout())} {}
  ~pipeline_layout() { D::destroy_pipeline_layout(m_pipeline_layout); }
...
template <class D> class shader_module {
public:
  shader_module(D &device, const spirv_file &file)
      : m_device{device}, m_shader_module{device.create_shader_module(file)} {}
  ~shader_module() { m_device.destroy_shader_module(m_shader_module); }

  auto get_shader_module() const { return m_shader_module; }
...
template <class D> class pipeline : public D {
public:
  pipeline(std::invocable<D &> auto &&generate_shader_module)
      : m_pipeline{D::create_pipeline(
            generate_shader_module(*this).get_shader_module(),
            D::get_pipeline_layout())} {}
  ~pipeline() { D::destroy_pipeline(m_pipeline); }
  auto get_pipeline() const { return m_pipeline; }

private:
  VkPipeline m_pipeline;
};

template <class D> class command_buffer : public D {
public:
  command_buffer()
      : m_command_buffer{D::allocate_command_buffer(D::get_command_pool())} {}
  auto get_command_buffer() const { return m_command_buffer; }
  void begin() {
  ...
  void bind_descriptor_sets(VkPipelineBindPoint bind_point,
                            ...
                            VkDescriptorSet descriptor_set) {
    vkCmdBindDescriptorSets(m_command_buffer, bind_point, layout, 0, 1,
  ...
  void pipeline_barrier(VkPipelineStageFlags src_stage_mask,
    ...
    vkCmdPipelineBarrier(m_command_buffer, src_stage_mask, dst_stage_mask,
  ...
  void clear_color_image(VkImage image, VkImageLayout layout,
    ...
    vkCmdClearColorImage(m_command_buffer, image, layout, clear_color,
                         range_count, ranges);
  }
...
template <class D> class add_storage_buffer : public D {
public:
  add_storage_buffer()
      : m_storage_buffer{D::create_buffer(D::get_queue_family_index(),
                                          D::get_storage_buffer_size(),
                                          VK_BUFFER_USAGE_STORAGE_BUFFER_BIT)} {
  }
  ~add_storage_buffer() { D::destroy_buffer(m_storage_buffer); }
  auto get_storage_buffer() const { return m_storage_buffer; }

private:
  VkBuffer m_storage_buffer;
};

template <class D> class add_storage_memory : public D {
public:
  add_storage_memory()
      : m_storage_memory{D::alloc_device_memory(
            D::get_memory_properties(), D::get_storage_buffer(),
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)} {}
  ~add_storage_memory() { D::free_device_memory(m_storage_memory); }
  auto get_storage_memory() const { return m_storage_memory; }

private:
  VkDeviceMemory m_storage_memory;
};

template <class D> class add_storage_memory_ptr : public D {
public:
  add_storage_memory_ptr()
      : m_storage_memory_ptr{
            D::map_device_memory(D::get_storage_memory(), 0, VK_WHOLE_SIZE)} {}
  ~add_storage_memory_ptr() { D::unmap_device_memory(D::get_storage_memory()); }
  auto get_storage_memory_ptr() const { return m_storage_memory_ptr; }
